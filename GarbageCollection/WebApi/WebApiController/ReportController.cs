    ...
    public class ReportController : ApiController
    {
        ...
        public IEnumerable<MainViewModel.SubscriptionViewModel> GetSubscriptionList([FromUri]  PagingParameterModel pagingparametermodel, int? customerid,string customerName="")
        {
            ...
            try
            {
                ...
                int TotalCount = count;
                int TotalPages = (int)Math.Ceiling(count / (double)PageSize);
                returnData = returnData.Skip((CurrentPage - 1) * PageSize).Take(PageSize).ToList();

                foreach (var item in returnData)
                {
                    query = String.Format("select * from fgetCuststmentSummaryForMobile("+ item.Subsid + ")");
                   item.totalDueData =  db.Database.SqlQuery<MainViewModel.TotalDueModel>(query).SingleOrDefault();
        ...
        public IEnumerable<MainViewModel.SubscriberDueViewModel> GetSubscriptionDueList([FromUri]  PagingParameterModel pagingparametermodel, int? customerid)
        {
            ...
            try
            {
                ...
                string query = String.Format("select COUNT(*) OVER() AS TotalCount, subsid,Subsno, CustomerName, CustomerType, LocationName, LedgerName, Debit, Credit, DueBalance, Status from[dbo].[fgetSubscriptionDueReport]('" + DateTime.Now + "') where custId= " + customerid);
                //string query = String.Format("select COUNT(*) OVER () AS TotalCount,SubsId,CollectorId,LocationID,Collectorname,CustId,Subscollid,subsno,LocationID,verifiedBy as SubsNo,CustomerName,LocationName,CollectionDate,CollectionAmt ,DiscountAmt,CollectionType,PostedBy   from fgetCollectionlist()  where verifiedby is null and CollectorId=" + collectorid + "and CollectionType=" + "'Mobile'");
        ...
        [HttpGet]
        [Route("subscriptionhistory")]
        public IEnumerable<ApiViewModel.HistoryModel> SubscriptionHistory([FromUri] PagingParameterModel pagingparametermodel, int? subsid)
        {
            ResponseMessage resMsg = new ResponseMessage();
            try
            {
                ...
                string query = String.Format("select * from fgetCuststmenthisteryForMobile("+ subsid + ")");
                //string query = String.Format("select COUNT(*) OVER () AS TotalCount,SubsId,CollectorId,LocationID,Collectorname,CustId,Subscollid,subsno,LocationID,verifiedBy as SubsNo,CustomerName,LocationName,CollectionDate,CollectionAmt ,DiscountAmt,CollectionType,PostedBy   from fgetCollectionlist()  where verifiedby is null and CollectorId=" + collectorid + "and CollectionType=" + "'Mobile'");

                List <ApiViewModel.HistoryModel> returnData = db.Database.SqlQuery<ApiViewModel.HistoryModel>(query).ToList();
